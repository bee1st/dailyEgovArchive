...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
			...
			<form id="noticeVO" name="noticeVO" method="post">
				...
				<div>
					<label for="noticeTitle">제목</label>
					<input type="text" id="noticeTitle" name="noticeTitle" value="<c:out value='${resultView.noticeTitle}' />"/>
				</div>			
				<div>
					<label for="noticeContents">내용</label>
					<textarea id="noticeContents" name="noticeContents" rows="6" cols="20"><c:out value='${resultView.noticeContents}' /></textarea>
				</div>
				<div>
					<label for="noticeTitle">등록자</label>
					<input type="text" id="register" name="register" value="<c:out value='${resultView.register}' />" />
				</div>
				<div>
					<input type="hidden" id="registedDate" name="registedDate" value="<c:out value='${resultView.registedDate}' />" />
				</div>
...
function fnUpdate() {
	...
	if(confirm("수정하시겠습니까?")) {
		$("#noticeVO").attr("action", "<c:url value='/notice/edit.do' />").submit();
	}
